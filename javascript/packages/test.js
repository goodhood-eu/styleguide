...
const collectErrors = (acc, line) => {
  ...
  if (notCheckedRules.length) {
    acc.push({ line, message: `Linter should check these rules: ${notCheckedRules.join(', ')}` });
  }

  if (extraCheckedRules.length) {
    acc.push({ line, message: `Linter should not check these rules: ${extraCheckedRules.join(', ')}` });
  }
